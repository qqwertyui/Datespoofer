...
int main(int argc, char **argv) {
  if(argc != 3) {
    fputs("Usage: datespoofer.exe <executable> <date> (e.g. \"5.09.2020 20:20:31\")\n", stderr);
  ...
  if(sscanf(date, "%d.%d.%d %d:%d:%d", &day, &month, &year, &hour, &minute, &second) < 0) {
  ...
  IMAGE_NT_HEADERS *nt_headers = (IMAGE_NT_HEADERS*)&(buffer.data()[dos_header->e_lfanew]);
  if(memcmp(&nt_headers->Signature, &IMAGE_NT_SIGNATURE_, sizeof(IMAGE_NT_SIGNATURE_)) != 0) {
    fputs("Invalid PE file\n", stderr);
    return Status::NOT_PE_ERROR;
  }
